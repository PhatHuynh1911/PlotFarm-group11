...
export async function getServiceTypes() {
  const result = await apiRequest('/YeuCauDichVu/types')
  return result.data
}
 
export async function createServiceRequest(payload, token) {
  return apiRequest('/YeuCauDichVu', { method: 'POST', body: JSON.stringify(payload), token })
}
...
export async function getServiceRequests(token) {
  const result = await apiRequest('/YeuCauDichVu/all', { token })
  return result.data
}
 
export async function updateServiceRequest(id, payload, token) {
  return apiRequest(`/YeuCauDichVu/${id}/status`, { method: 'PATCH', body: JSON.stringify(payload), token })
}
 
export async function submitContact(payload) {
  return apiRequest('/LienHeTuVan', { method: 'POST', body: JSON.stringify(payload) })
}
...
export async function getUserServiceRequests(userId, token) {
  const result = await apiRequest(`/YeuCauDichVu/user/${userId}`, { token })
  return result.data
}
