...
function FarmerPage({ user, onLogout }) {
  ...
  return <main className="dashboard-page farmer-dashboard">
    <header className="dashboard-header">
      ...
      <div className="dashboard-account"><span>{user.name}<small>Nông dân PlotFarm</small></span><button className="dashboard-logout" onClick={onLogout}>Đăng xuất</button></div>
    </header>
    <section className="dashboard-shell">
      ...
      <nav className="farmer-tabs" aria-label="Chức năng nông dân">
        {[['plots', 'Ô đất được phân công'], ['journal', 'Nhật ký canh tác'], ['requests', `Yêu cầu chăm sóc${activeRequests ? ` (${activeRequests})` : ''}`], ['cameras', 'Camera / thiết bị'], ['harvest', 'Xác nhận thu hoạch']].map(([id, label]) => <button key={id} className={activeTab === id ? 'active' : ''} onClick={() => setActiveTab(id)}>{label}</button>)}
      </nav>
