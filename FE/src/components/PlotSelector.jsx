...
function PlotSelector({
  ...
  return (
    <>
      <section className="plots-section section-shell" id="plots">
        <div className="section-intro">
          <div>
            <p className="eyebrow">Ô ĐẤT ĐANG SẴN SÀNG</p>
            <h2>
              Chọn một không gian
              <br />
              <em>cho mùa xanh mới.</em>
            </h2>
          </div>
          <button
        ...
        <p className="section-note listings-note">
          Tạo một khởi đầu mới với ô đất riêng, được chăm sóc và cập nhật minh
          bạch trong suốt mùa vụ.
        </p>
        {loading && (
        ...
        <div className="listing-grid">
          {availablePlots.slice(0, 3).map((plot, index) => (
            <article className="listing-card" key={plot.id}>
              <div
              ...
              >
                ...
                <span className="listing-heart">♡</span>
              </div>
              <div className="listing-body">
                <div className="listing-price">
                  <strong>{plot.price.toLocaleString("vi-VN")}đ</strong>
                  <small>/ tháng</small>
                </div>
                <p>{plot.location}</p>
                <div className="listing-meta">
                  <span>
                    {plot.code} · {plot.area}m²
                  </span>
        ...
        <div className="listing-banner">
          <div>
            ...
            <h3>Giữ chỗ cho một mùa xanh</h3>
            <p>Chọn ô đất phù hợp và để đội ngũ PlotFarm đồng hành cùng bạn.</p>
          </div>
          <button className="primary-button" onClick={onReserve}>
            Giữ ô đất {selectedPlot} <span>→</span>
          </button>
        </div>
      </section>
      {showMap && (
      ...
      {detailPlot && (
        <GardenDetailView
          plot={detailPlot}
          occupied={detailPlot.status !== "trong"}
          onClose={closeDetail}
          onReserve={onReserve}
        />
      )}
