...
function UserPage({ user, token, onLogout }) {
  ...
  const submitSupport = async (event) => {
    ...
    const rental = rentals.find((item) => String(item.ma_hop_dong) === String(selectedCareRental))
    try {
  ...
  if (activeTab === 'gardens') return workspace(<><div className="user-summary"><div><span>Hợp đồng của tôi</span><strong>{rentals.length}</strong></div><div><span>Đang canh tác</span><strong>{rentals.filter((item) => item.trang_thai_hop_dong === 'hieu_luc').length}</strong></div><div><span>Email tài khoản</span><strong className="user-email">{user.email}</strong></div></div><section className="dashboard-panel rental-panel"><div className="panel-heading"><div><p className="eyebrow">MY PLOTS</p><h2>Những ô đất đang thuê</h2></div><button className="dashboard-link-button" onClick={() => setActiveTab('find')}>Khám phá ô đất <span>→</span></button></div>{loading && <p className="loading-state">Đang tải khu vườn...</p>}{!loading && rentals.length === 0 && <p className="empty-state">Bạn chưa có hợp đồng nào. Hãy chọn một ô đất cho mùa vụ đầu tiên.</p>}<div className="my-plot-grid">{rentals.map((rental) => <article className="my-plot-card" key={rental.ma_hop_dong}><div className="my-plot-image" style={rental.hinh_anh_o_dat ? { backgroundImage: `url(${rental.hinh_anh_o_dat})` } : undefined}><span>{rental.so_hieu_o}</span></div><div className="my-plot-body"><p className="plot-status">{rental.trang_thai_hop_dong === 'hieu_luc' ? 'Đang thuê' : rental.trang_thai_hop_dong}</p><h3>{rental.ten_o_dat}</h3><p>{rental.ten_cay_trong || 'Chưa chọn cây trồng'} · Farmer: {rental.ten_nong_dan || 'Đang chờ phân công'}</p><div className="plot-date-row"><span>Kết thúc thuê</span><strong>{formatDate(rental.ngay_ket_thuc)}</strong></div><div className="remaining-days"><b>{daysRemaining(rental.ngay_ket_thuc)}</b><span>ngày còn lại</span></div><button className="outline-button" onClick={() => { setSelectedJournalRental(String(rental.ma_hop_dong)); setActiveTab('journal') }}>Xem nhật ký canh tác →</button></div></article>)}</div></section></>)

  if (activeTab === 'journal') {
    const selectedJournals = journals.filter((item) => String(item.ma_hop_dong) === String(selectedJournalRental))
    const selectedRental = rentals.find((rental) => String(rental.ma_hop_dong) === String(selectedJournalRental))
    return workspace(<section className="dashboard-panel timeline-panel"><div className="panel-heading"><div><p className="eyebrow">DIARY TIMELINE</p><h2>Nhật ký canh tác</h2></div><div className="panel-heading-actions"><select value={selectedJournalRental} onChange={(event) => setSelectedJournalRental(event.target.value)} aria-label="Chọn ô đất xem nhật ký">{rentals.map((rental) => <option key={rental.ma_hop_dong} value={rental.ma_hop_dong}>{rental.so_hieu_o} · {rental.ten_o_dat}</option>)}</select><span className="result-count">{selectedJournals.length} cập nhật</span></div></div>{selectedJournals.length === 0 ? <p className="empty-state">Chưa có nhật ký từ nông dân cho ô {selectedRental?.so_hieu_o || 'đất này'}.</p> : <div className="diary-timeline">{selectedJournals.map((item) => { const images = journalImages(item); return <article className="diary-card" key={item.ma_nhat_ky}><div className="timeline-dot" /><div className="diary-date">{formatDate(item.ngay_ghi_nhat_ky || item.ngay_tao)}</div><div className="diary-content"><div><p className="plot-status">{item.so_hieu_o || selectedRental?.so_hieu_o || 'Khu vườn của bạn'} · {item.giai_doan_sinh_truong || 'Canh tác'}</p><h3>{item.tieu_de || item.cong_viec_da_lam}</h3><p className="diary-meta">{item.ten_nong_dan || 'Nông dân phụ trách'} · {item.thoi_tiet || 'Đang cập nhật thời tiết'}</p><p>{item.noi_dung || item.ghi_chu_chi_tiet || 'Chưa có ghi chú chi tiết.'}</p>{item.loai_phan_bon_da_dung && <small>Phân bón: {item.loai_phan_bon_da_dung}</small>}</div>{images[0] && <img src={images[0]} alt={`Nhật ký ${item.tieu_de || ''}`} />}</div></article> })}</div>}</section>)
  }
  ...
  return <main className="dashboard-page user-dashboard"><header className="dashboard-header"><a className="brand" href="/"><span className="brand-mark">PF</span><span>plot<span>farm</span></span></a><nav className="workspace-nav" aria-label="Điều hướng khách hàng">{tabs.map(([id, label]) => <button key={id} className={activeTab === id ? 'active' : ''} onClick={() => setActiveTab(id)}>{label}</button>)}</nav><AccountMenu user={user} roleLabel="Khách hàng PlotFarm" onProfile={() => setActiveTab('profile')} onLogout={onLogout} /></header><section className="dashboard-shell"><p className="eyebrow">KHU VƯỜN CỦA BẠN</p><h1>Chào mừng, <em>{user.name.split(' ').pop()}.</em></h1><p className="dashboard-lead">Quản lý mùa vụ, chăm sóc và nông sản của bạn trong một nơi.</p><nav className="user-tabs" aria-label="Điều hướng tài khoản">{tabs.map(([id, label]) => <button key={id} className={activeTab === id ? 'active' : ''} onClick={() => setActiveTab(id)}>{label}</button>)}</nav>{notice && <p className="dashboard-notice" role="status">{notice}</p>}
    ...
    {activeTab === 'support' && <section className="dashboard-panel form-panel"><div><p className="eyebrow">YÊU CẦU CHĂM SÓC</p><h2>Để nông dân chăm vườn cùng bạn</h2><p>Gửi yêu cầu bón phân, tỉa cành, tưới nước hoặc xử lý sâu bệnh.</p></div>{supportSent ? <p className="form-success">Yêu cầu đã được gửi tới nông dân phụ trách.</p> : <form onSubmit={submitSupport}><select name="rentalId" value={selectedCareRental} onChange={(event) => setSelectedCareRental(event.target.value)} required><option value="">Chọn ô đất</option>{rentals.map((rental) => <option key={rental.ma_hop_dong} value={rental.ma_hop_dong}>{rental.so_hieu_o} · {rental.ten_nong_dan ? `Farmer: ${rental.ten_nong_dan}` : 'Chưa có farmer'}</option>)}</select><p className="assigned-farmer">Farmer phụ trách: <strong>{rentals.find((rental) => String(rental.ma_hop_dong) === String(selectedCareRental))?.ten_nong_dan || 'Sẽ hiển thị sau khi admin phân công'}</strong></p><select name="serviceType" required><option value="">Chọn loại hỗ trợ</option>{serviceTypes.map((service) => <option key={service.ma_loai_dich_vu} value={service.ma_loai_dich_vu}>{service.ten_dich_vu}</option>)}</select><input name="schedule" required placeholder="Thời gian mong muốn" /><textarea name="note" required placeholder="Mô tả tình trạng hoặc ghi chú cho nông dân" /><button className="primary-button">Gửi yêu cầu <span>→</span></button></form>}<div className="request-list">{serviceRequests.map((request) => <article className="care-request" key={request.ma_yeu_cau}><div><strong>{request.so_hieu_o} · {request.ten_dich_vu}</strong><p>Farmer: {request.ten_nong_dan_xu_ly || 'Chưa có'} · {request.trang_thai_xu_ly}</p>{request.phan_hoi_cua_nha_vuon && <small>Phản hồi: {request.phan_hoi_cua_nha_vuon}</small>}</div></article>)}</div></section>}
  ...
  </section>{error && activeTab !== 'gardens' && <p className="dashboard-error" role="alert">{error}</p>}{selectedPlot && <div className="booking-backdrop"><form className="booking-modal" onSubmit={bookingStep === 'details' ? continueToPayment : submitBooking}><button type="button" className="modal-close" onClick={closeBooking}>×</button>{bookingStep === 'details' ? <><p className="eyebrow">BƯỚC 1 / 2 · THUÊ Ô {selectedPlot.code}</p><h2>Đặt mùa vụ của bạn</h2><p>{selectedPlot.area}m² · {selectedPlot.soil} · {formatMoney(selectedPlot.price)}/tháng</p><label>Thời hạn thuê<select value={booking.duration} onChange={(event) => setBooking({ ...booking, duration: event.target.value })}><option value="3">3 tháng · {formatMoney(selectedPlot.price * 3)}</option><option value="6">6 tháng · {formatMoney(selectedPlot.price * 6)}</option><option value="12">12 tháng · {formatMoney(selectedPlot.price * 12)}</option></select></label><label>Hạt giống / loại cây<select value={booking.crop} onChange={(event) => setBooking({ ...booking, crop: event.target.value })}><option>Rau xà lách</option><option>Cà chua bi</option><option>Dâu tây</option><option>Rau gia vị</option></select></label><button className="primary-button booking-submit">Xem lại & thanh toán <span>→</span></button></> : <><p className="eyebrow">BƯỚC 2 / 2 · XÁC NHẬN & THANH TOÁN</p><h2>Xác nhận thông tin thuê</h2><div className="booking-summary"><div><span>Ô đất</span><strong>{selectedPlot.code} · {selectedPlot.location}</strong></div><div><span>Mùa vụ</span><strong>{booking.crop} · {booking.duration} tháng</strong></div><div><span>Tạm tính</span><strong>{formatMoney(selectedPlot.price * Number(booking.duration))}</strong></div></div><label>Phương thức thanh toán<select value={booking.payment} onChange={(event) => setBooking({ ...booking, payment: event.target.value })}><option>Chuyển khoản</option><option>Ví điện tử</option><option>Thẻ quốc tế</option></select></label><p className="payment-note">Thanh toán được xác nhận an toàn. Sau khi gửi, hệ thống sẽ tạo hợp đồng và giữ ô đất cho bạn.</p><div className="booking-actions"><button type="button" className="outline-button" onClick={() => setBookingStep('details')}>Quay lại</button><button className="primary-button booking-submit" disabled={bookingSubmitting}>{bookingSubmitting ? 'Đang gửi...' : 'Xác nhận thuê ô'} <span>→</span></button></div></>}</form></div>}</main>
}
