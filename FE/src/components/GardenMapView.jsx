...
function GardenMapView({ plots, occupiedPlots, onSelectPlot, onClose }) {
  ...
  const visiblePlots = plots.filter((plot) => {
    const matchesQuery = plot.toLowerCase().includes(query.toLowerCase())
    const isOccupied = occupiedPlots.includes(plot)
  ...
  return <div className="garden-map-overlay" role="dialog" aria-modal="true" aria-label="Bản đồ khu vườn" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    ...
    <div className="garden-map-content">
      <aside className="map-listings">
        ...
        <div className="map-list-grid">{visiblePlots.map((plot, index) => { const isOccupied = occupiedPlots.includes(plot); return <article className="map-listing" key={plot} onClick={() => !isOccupied && onSelectPlot(plot)}><div className={`map-listing-image map-listing-image-${(index % 4) + 1}`}><span className={isOccupied ? 'map-listing-status occupied' : 'map-listing-status'}><i /> {isOccupied ? 'Đã thuê' : 'Đang trống'}</span><span className="map-listing-heart">♡</span></div><div className="map-listing-body"><strong>{index % 3 === 0 ? '490.000đ' : index % 3 === 1 ? '650.000đ' : '820.000đ'}<small>/ tháng</small></strong><p>Vườn Phúc Lộc, Đà Nẵng</p><span>{plot} · 24m² &nbsp; <b>●</b> Có chăm sóc</span></div></article> })}</div>
      </aside>
      <section className="garden-map-canvas" aria-label="Bản đồ vệ tinh khu vườn"><div className="map-location-label">VƯỜN PHÚC LỘC<br /><small>ĐÀ NẴNG</small></div>{plots.map((plot, index) => <button key={plot} type="button" className={`map-marker ${occupiedPlots.includes(plot) ? 'occupied' : ''} marker-${index + 1}`} onClick={() => !occupiedPlots.includes(plot) && onSelectPlot(plot)} aria-label={`Chọn ô ${plot}`}>{plot}</button>)}<div className="map-zoom"><button type="button">+</button><button type="button">−</button></div><div className="map-compass">N</div></section>
    </div>
  </div>
}
